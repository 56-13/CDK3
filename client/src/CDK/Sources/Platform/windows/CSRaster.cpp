...
CSVector2 CSRaster::characterSize(const uchar* cc, int cclen, const CSFont* font) {
	...
	foreach (FT_Face, face, faces) {
		...
		while (i < cclen) {
			uint code = string::toCodePoint(cc, i);
	...
	return CSVector2::Zero;
}

void* CSRaster::createRawWithCharacter(const uchar* cc, int cclen, const CSFont* font, int& width, int& height, float& offy) {
	CSArray<FT_Face>* faces = (CSArray<FT_Face>*)font->handle();
...
void* CSRaster::createRawWithBitmap(const void* source, int len, int& width, int& height) {
	...
	for (int y = height - 1; y >= 0; y--) {
		for (int x = 0; x < width; x++) {
			...
			raster[i++] = color.rgbReserved;		//TODO:CHECK THIS IS ALPHA CHANNEL
		}
	}
...
bool CSRaster::saveBitmapWithRaw(const char* path, const void* raw, int width, int height) {
	FIBITMAP* img = FreeImage_Allocate(width, height, 24);
	if (!img) return false;
	
	const byte* ptr = (const byte*)raw;
	...
	for (int y = 0; y < height; y++) {		//flip?
		for (int x = 0; x < width; x++) {
			...
			color.rgbGreen = *ptr++;
			color.rgbBlue = *ptr++;
			color.rgbReserved = *ptr++;		//TODO:CHECK THIS IS ALPHA CHANNEL
