...
const void* CSCrypto::encrypt(const void* data, int& length, const char* key) {
	int keyBit;
	switch (strlen(key)) {
		case 32:
			keyBit = 256;
	...
	int body_len = length / AES_BLOCK_SIZE * AES_BLOCK_SIZE;
	int out_len = body_len;
...
const void* CSCrypto::decrypt(const void* data, int& length, const char* key) {
	int keyBit;
	switch (strlen(key)) {
		case 32:
			keyBit = 256;
...
string CSCrypto::sha1(const void* data, int length) {
	SHA_CTX context;
