...
void CSURLRequestBridge::setHeaderField(void* handle, const char* name, const char* value) {
	static_cast<CSURLRequestHandle*>(handle)->headerFields.setObject(string(name), string(value));
}

const char* CSURLRequestBridge::headerField(void* handle, const char* name) {
	return static_cast<CSURLRequestHandle*>(handle)->headerFields.objectForKey(string(name));
}

const CSDictionary<string, string>* CSURLRequestBridge::headerFields(void* handle) {
	return &static_cast<CSURLRequestHandle*>(handle)->headerFields;
}

void CSURLRequestBridge::setBody(void* handle, const void* data, int length) {
	CSURLRequestHandle* h = static_cast<CSURLRequestHandle*>(handle);
