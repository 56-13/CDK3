...
static const int GzipBufferIncrement = 65536;

void* CSFile::createCompressedRawWithData(const void* src, int srcLength, int& destLength) {
    z_stream stm = {};
    ...
    for (; ; ) {
        ...
        switch (rtn) {
            ...
            case Z_STREAM_END:
				...
				if (destLength < bufferSize) dest = realloc(dest, destLength);
                goto exit;
            default:
                free(dest);
                dest = NULL;
...
void* CSFile::createRawWithCompressedData(const void* src, int srcLength, int destOffset, int& destLength) {
    z_stream stm = {};
    ...
    int bufferSize = destLength ? destLength : srcLength * GzipUncompressBufferRate;
...
void* createRawWithCompressedContentOfFile(const char* path, int offset, int& length) {
	...
	if (offset) gzseek(fp, offset, SEEK_CUR);
    ...
    if (data) {
        if (length) {
			...
			if (length < bufferSize) data = realloc(data, length);
        }
    }
    else length = 0;
...
void* createRawWithContentOfFile(const char* path, int offset, int& length) {
    ...
    if (fp) {
        ...
        if (offset < end) {
            ...
            data = malloc(length);
            if (data) length = fread(data, sizeof(byte), length, fp);
            else length = 0;
        }
        else length = 0;
        fclose(fp);
...
bool writeRawToFile(const char* path, const void* data, int length) {
    ...
    if (fp) {
        if (length) {
            ...
            if (complete) complete = (fflush(fp) == 0);
        }
        else complete = true;
        fclose(fp);
    }
