...
CSAndroidView::CSAndroidView(int width, int height, int bufferWidth, int bufferHeight, int horizontalEdge, int verticalEdge) {
...
void CSAndroidView::resizeBuffer(int bufferWidth, int bufferHeight) {
    _graphics->target()->resize(bufferWidth, bufferHeight);
}

void CSAndroidView::reload() {
    int bufferWidth = _graphics->target()->width();
    int bufferHeight = _graphics->target()->height();
    _graphics->release();
    _graphics = new CSGraphics(CSRenderTarget::target(bufferWidth, bufferHeight, true));
}
