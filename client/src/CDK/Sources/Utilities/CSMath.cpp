...
bool CSMath::nearEqual(float a, float b) {
...
void CSMath::gaussKernel(int radius, float sigma, float* result) {
    double c = 1.0 / (2.0 * DoublePi * sigma);
    float sum = result[0] = c;
    for (int i = 1; i <= radius; i++) {
        double e = -(i * i) / (2.0 * sigma);
        float v = c * CSMath::exp(e);
        result[i] = v;
        sum += v * 2;
    }
