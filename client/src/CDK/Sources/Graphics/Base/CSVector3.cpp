...
const CSVector3 CSVector3::Zero(0, 0, 0);
const CSVector3 CSVector3::UnitX(1, 0, 0);
const CSVector3 CSVector3::UnitY(0, 1, 0);
const CSVector3 CSVector3::UnitZ(0, 0, 1);
const CSVector3 CSVector3::One(1, 1, 1);
...
void CSVector3::cross(const CSVector3& left, const CSVector3& right, CSVector3& result) {
...
void CSVector3::unproject(const CSVector3& vector, float x, float y, float width, float height, float minZ, float maxZ, const CSMatrix& worldViewProjection, CSVector3& result) {
    CSVector3 v;
    CSMatrix m;
    CSMatrix::invert(worldViewProjection, m);
    
    v.x = (((vector.x - x) / width) * 2.0f) - 1.0f;
    v.y = -((((vector.y - y) / height) * 2.0f) - 1.0f);
    v.z = (vector.z - minZ) / (maxZ - minZ);
    
    transformCoordinate(v, m, result);
}
...
bool CSVector3::nearEqual(const CSVector3& a, const CSVector3& b) {
    return
