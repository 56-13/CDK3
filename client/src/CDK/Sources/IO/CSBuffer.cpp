...
CSBuffer::CSBuffer() : _data(new CSData()) {
    
}

CSBuffer::CSBuffer(int capacity) : _data(new CSData(capacity)) {

}

CSBuffer::CSBuffer(CSData* data) : _data(retain(data)) {
    CSAssert(data);
}

CSBuffer::CSBuffer(void* data, int length, bool copy) : _data(new CSData(data, length, copy)) {
...
CSBuffer* CSBuffer::createWithContentOfFile(const char* path, int offset, int length, bool compression) {
...
CSBuffer* CSBuffer::remainedBuffer() const {
    int remainedLength = _data->length() - _position;
...
void CSBuffer::read(void* data, int size) {
    CSAssert(size > 0 && _position <= _data->length() - size, "position overflow");
    memcpy(data, (const byte*)_data->bytes() + _position, size);
    _position += size;
}

CSData* CSBuffer::readData(int length) {
...
half CSBuffer::readHalf() {
    CSAssert(_position <= _data->length() - 2, "position overflow");
    half v = bytesToHalf((const byte*)_data->bytes() + _position);
    _position += 2;
    return v;
}
...
string CSBuffer::readString() {
    return string(this);
}

CSLocaleString* CSBuffer::readLocaleString() {
    return CSLocaleString::localeStringWithBuffer(this);
}

void CSBuffer::write(const void* data, int size) {
...
void CSBuffer::writeHalf(half v) {
    _data->appendBytes(&v.raw, 2);
}
...
void CSBuffer::writeLength(int len) {
    for (int i = 21; i > 0; i -= 7) {
        int current = (len >> i) & 0x7f;
        
        if (current) writeByte(current | 0x80);
    }
    writeByte(len & 0x7f);
}

void CSBuffer::writeString(const char* str) {
    string::writeTo(this, str);
}

void CSBuffer::writeString(const string& str) {
    str.writeTo(this);
}
