...
static float __totalWaitingTime = 0;
#endif
...
bool CSLock::trylock(int op) {
	...
	if (op) {
		synchronized(__waitingTimes) {
			...
			if (pv) *pv += delay;
			else __waitingTimes->setObject(op, delay);
		}
	}
...
void CSLock::unlock() {
    ...
    CSAssert(pthread_exists(owner) && pthread_equal(owner, pthread_self()));
    if (--_ownCount == 0) pthread_empty(_owner);
#endif
...
bool CSLock::assertOnActive(bool abort) const {
    ...
    if (pthread_exists(owner) && pthread_equal(owner, pthread_self())) return true;
    else {
