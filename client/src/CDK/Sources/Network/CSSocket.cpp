...
CSSocket::CSSocket(const char* host, int port, const Delegate& delegate) :
    _host(strdup(host)),
    _port(port),
    _delegate(delegate),
    _task(NULL),
...
void CSSocket::open() {
	...
	error = getaddrinfo(_host, string::cstringWithFormat("%d", _port), &hints, &res0);
	...
	_task = CSTask<void>::task(this, &CSSocket::update, 0, true);

	CSThread::mainThread()->run(_task);
