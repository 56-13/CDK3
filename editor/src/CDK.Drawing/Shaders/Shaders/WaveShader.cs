...
{
    public class WaveShader : IDisposable
    {
        private ProgramBranch _programs;

        public WaveShader()
        {
            var commonShaderCode = File.ReadAllText(Path.Combine("Resources", "wave.glsl"));
            var vertexShaderCode = File.ReadAllText(Path.Combine("Resources", "wave_vs.glsl"));
            var fragmentShaderCode = File.ReadAllText(Path.Combine("Resources", "wave_fs.glsl"));
        ...
        private struct UniformData : IEquatable<UniformData>
        {
            ...
            public override int GetHashCode()
            {
                ...
                hash.Combine(Thickness.GetHashCode());
                return hash;
            }
        ...
        public void Draw(GraphicsApi api, Texture screenTexture, in Matrix4x4 worldViewProjection, in Vector3 center, float radius, float thickness)
        {
