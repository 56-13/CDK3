...
namespace CDK.Assets.Components
{
	public class MultiselectTreeView : TreeView
	{
        private struct NodeColor
        {
            ...
            public Color BackColor { private set; get; }

            public NodeColor(Color foreColor, Color backColor) : this()
            {
                ForeColor = foreColor;
                BackColor = backColor;
            }
        ...
        private Dictionary<TreeNode, NodeColor> nodeColors;
		...
		{
            nodeColors = new Dictionary<TreeNode, NodeColor>();
		...
		private void ClearSelectedNodes()
		{
			foreach ( TreeNode node in m_SelectedNodes )
			{
                node.BackColor = nodeColors[node].BackColor;
                node.ForeColor = nodeColors[node].ForeColor;
			}
            nodeColors.Clear();
		...
		private void ToggleNode( TreeNode node, bool bSelectNode )
		{
			if( bSelectNode )
			{
                ...
                {
                    nodeColors.Add(node, new NodeColor(node.ForeColor, node.BackColor));
                }
